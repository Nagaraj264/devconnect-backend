...
import { Heart, MessageSquare, Share2, Clock } from 'lucide-react'; // 👈 Use nice icons!

function PostCard({ post }) { // 🚀 1. Accept the single 'post' prop!
  // 🚀 2. Safely de-structure our node data
  const { 
    author, 
    title, 
    content, 
  ...
  const initials = author?.name?.charAt(0) || author?.username?.charAt(0) || "?";
    ...
    <article className="bg-surface-container/50 rounded-[2rem] p-8 border border-white/5 hover:border-primary/20 transition-all group shadow-inner">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 text-[10px] font-mono text-on-surface-variant/30 uppercase tracking-tighter">
           <Clock size={12} />
           <span>2h ago</span> {/* Later we can use a date-formatter here! */}
        </div>
      </div>

      <div className="space-y-4 mb-6">
        <h2 className="text-2xl font-black text-on-surface tracking-tight leading-tight">{title}</h2>
        <p className="text-on-surface-variant/70 text-sm leading-relaxed whitespace-pre-wrap">{content}</p>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-white/5 text-on-surface-variant/50">
        <div className="flex gap-4">
          ...
          <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface/50 hover:bg-surface hover:text-primary transition-all">
